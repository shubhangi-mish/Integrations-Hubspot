...
from integrations.hubspot import authorize_hubspot, get_hubspot_credentials, get_items_hubspot, oauth2callback_hubspot, load_tokens_from_json
...
@app.get('/integrations/hubspot/tokens/{org_id}/{user_id}')
async def get_hubspot_saved_tokens(org_id: str, user_id: str):
    """
    Get saved OAuth tokens from JSON file
    """
    tokens = load_tokens_from_json(user_id, org_id)
    if tokens:
        return {"tokens": tokens, "source": "json_file"}
    else:
        raise HTTPException(status_code=404, detail="No saved tokens found")
